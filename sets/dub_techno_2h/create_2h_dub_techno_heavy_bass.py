...
def main():
    ...
    for i, name in enumerate(track_names):
        if i < 6:  # Tracks 0-5 are MIDI
            ...
            if result.get("status") == "success":
                print(f"      ✓ Created MIDI track {i}: {name}")
            else:
                print(f"      ✗ Error creating track {i}: {result.get('message')}")
        else:  # Tracks 6-7 are Audio (send tracks)
            result = send_command("create_audio_track", {"index": -1})
            if result.get("status") == "success":
                print(f"      ✓ Created audio track {i}: {name}")
            else:
                print(f"      ✗ Error creating track {i}: {result.get('message')}")
        time.sleep(0.2)
    ...
    for i, name in enumerate(track_names):
        ...
        if result.get("status") == "success":
            print(f"      ✓ Track {i} named: {name}")
        else:
            print(f"      ✗ Error naming track {i}: {result.get('message')}")
        time.sleep(0.1)
    ...
    if result.get("status") == "success":
        print(f"      ✓ Tempo set to 114 BPM")
    else:
        print(f"      ✗ Error setting tempo: {result.get('message')}")
    ...
    for track_idx, name, uri in instruments:
        ...
        if result.get("status") == "success":
            print(f"      ✓ Track {track_idx} ({name}): Loaded instrument")
        else:
            print(
                f"      ✗ Track {track_idx} ({name}): {result.get('message', 'Error')}"
            )
        time.sleep(0.5)
    ...
    for track_idx, name, uri in effects:
        ...
        if result.get("status") == "success":
            print(f"      ✓ Track {track_idx} ({name}): Loaded effect")
        else:
            print(
                f"      ✗ Track {track_idx} ({name}): {result.get('message', 'Error')}"
            )
    ...
    for track_idx, volume in volumes.items():
        ...
        if result.get("status") == "success":
            ...
            print(f"      ✗ Track {track_idx} volume error: {result.get('message')}")
        time.sleep(0.1)
