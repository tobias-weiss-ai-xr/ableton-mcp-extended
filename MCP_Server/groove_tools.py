...
# Module-level reference set by register_groove_tools
_get_ableton_connection = None


def register_groove_tools(mcp: FastMCP, get_ableton_connection):
    """Register groove template MCP tools."""
    global _get_ableton_connection
    _get_ableton_connection = get_ableton_connection

    # Register existing module-level functions as MCP tools
    mcp.tool()(list_groove_templates)
    mcp.tool()(apply_groove_to_clip)
    mcp.tool()(remove_groove_from_clip)
    mcp.tool()(set_global_groove_amount)
...
def list_groove_templates(ctx: Context) -> str:
    ...
    """
    try:
        ableton = _get_ableton_connection()
