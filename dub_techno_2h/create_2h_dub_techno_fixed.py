...
# Get the new track index (it should be the last one)
...
send_command("set_track_name", {"track_index": bass_track_index, "name": "Sub-bass"})
...
send_command("set_track_name", {"track_index": delay_track_index, "name": "Dub Delays"})
...
for clip_idx, clip_data in enumerate(delay_clips):
    send_command(
        "create_clip",
        {"track_index": delay_track_index, "clip_index": clip_idx, "length": 4.0},
    )
    send_command(
        "add_notes_to_clip",
        {
...
print(f"  Track {bass_track_index} (Sub-bass):    {len(bass_clips)} clips")
...
print(f"  Track {delay_track_index} (Dub Delays):  {len(delay_clips)} clips")
print(
